    ...
    for v, visit in enumerate(visits):
        ...
        if field_in_contest.upper() != 'GOODSN':
            ...
            if (ref_filter_1.lower() in filt1) or (ref_filter_2.lower() in filt1):
                ...
                grism_index_1= np.where((basenames == basename) & (ref_grism_1.lower() in filter_names))
                grism_index_2= np.where((basenames == basename) & (ref_grism_2.lower() in filter_names))
