...
def parse():
    ...
    parser.add_argument('-PATH_TO_SCRIPTS', '--PATH_TO_SCRIPTS', default = '/Users/rsimons/Dropbox/git/clear_local', help = 'path to scripts directory')
        ...
        if os.path.isfile(field + '_' + '%.5i.beams.fits'%id):
            ...
            if pfit != None:
                ...
                try:
                    ...
                    fig.savefig('{0}_{1:05d}_new.stack.png'.format(field, id))
                    hdu.writeto('{0}_{1:05d}_new.stack.fits'.format(field, id), clobber=True)
                except:
                    pass
                ...
                try:
                    out = grizli.fitting.run_all(
                        ...
                        dz=[0.004, 0.0005], 
                        fitter='nnls',
                        group_name=field + '_new',# + '_%i'%phot_scale_order,
